"""
GRU Autoencoder + Batch Normalization Training
with proper scheduler, early stopping, and gradient clipping.

Similar to train_gru_ae_cn_gn.py but uses BN instead of CN.
"""
...
from models.gru_bn_ae import GRU_Autoencoder, GRU_FeatureClassifier
...
def train_classifier(clf, train_loader, val_loader, device, epochs=30, lr=5e-4,
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_val_acc:
            ...
            save_path = 'checkpoints/best_model_ae_bn.pth'
            if checkpoint_dir is not None:
                save_path = str(Path(checkpoint_dir) / 'best_model_ae_bn.pth')
            torch.save({
                'epoch': epoch,
...
def main():
    ...
    exp_name = f'gru_ae_bn_{timestamp}'
    ...
    print("="*70)
    print("GRU AUTOENCODER + BATCH NORMALIZATION TRAINING")
    print("="*70)
    ...
    final_exp_path = exp_ckpt_dir / 'gru_ae_bn_final.pth'
    torch.save({
    ...
    print(f"Best model saved to: {exp_ckpt_dir / 'best_model_ae_bn.pth'}")
