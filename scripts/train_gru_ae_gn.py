"""
GRU Autoencoder + Group Normalization Training
with proper scheduler, early stopping, and gradient clipping.

Similar to train_gru_ae_cn_gn.py but uses GN instead of CN.
"""
...
from models.gru_gn_ae import GRU_Autoencoder, GRU_FeatureClassifier
...
def train_classifier(clf, train_loader, val_loader, device, epochs=30, lr=5e-4,
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_val_acc:
            ...
            # FIX: pastikan directory ada sebelum save
            save_path = 'checkpoints/best_model_ae_gn.pth'
            if checkpoint_dir is not None:
                save_path = str(Path(checkpoint_dir) / 'best_model_ae_gn.pth')
            Path(save_path).parent.mkdir(parents=True, exist_ok=True)  # <-- FIX mkdir
...
def main():
    ...
    exp_name = f'gru_ae_gn_{timestamp}'
    ...
    print("="*70)
    print("GRU AUTOENCODER + GROUP NORMALIZATION TRAINING")
    print("="*70)
    ...
    final_exp_path = exp_ckpt_dir / 'gru_ae_gn_final.pth'
    torch.save({
    ...
    # ===== FIX: LOAD BEST MODEL SEBELUM EVALUASI FINAL =====
    best_ckpt_path = exp_ckpt_dir / 'best_model_ae_gn.pth'
    ...
    print(f"Best model saved to: {exp_ckpt_dir / 'best_model_ae_gn.pth'}")
